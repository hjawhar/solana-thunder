...
use thunder_core::{
    ...
    SwapContext, SwapDirection, calculate_price_impact_bps, constant_product_swap,
};
...
impl MeteoraDAMMMarket {
    ...
    fn calculate_damm_output(
        ...
        match &self.pool.curve_type {
            CurveType::ConstantProduct => match direction {
            ...
            CurveType::Stable { amp, .. } => {
                // Simplified stable curve: base constant-product adjusted by amplification factor.
...
impl Market for MeteoraDAMMMarket {
    fn metadata(&self) -> Result<PoolMetadata, GenericError> {
        ...
        Ok(PoolMetadata {
            ...
            quote_mint: self.pool.token_b_mint,
            base_mint: self.pool.token_a_mint,
    ...
    fn required_accounts(
        ...
        let (source_mint, destination_mint) = match direction {
            SwapDirection::Buy => (self.pool.token_b_mint, self.pool.token_a_mint),
            SwapDirection::Sell => (self.pool.token_a_mint, self.pool.token_b_mint),
        };
...
impl MeteoraDAMMV2Market {
    ...
    /// Convert V2 Q128 fixed-point sqrt_price to a regular price.
    ...
    fn calculate_base_fee_bps(&self) -> u64 {
        self.pool.pool_fees.base_fee.cliff_fee_numerator / 100
    }
